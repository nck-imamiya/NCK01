...
import { Camera, Users, Grid, Scissors, Puzzle, Layers, FlaskConical, ImageIcon, ImageOff, Beaker } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-16">
          <div className="flex items-center justify-between">
            ...
            <button 
              ...
              className="p-3 bg-white/50 backdrop-blur rounded-full text-indigo-300 hover:text-indigo-600 hover:bg-white transition-all shadow-sm"
              title="デザインテスト画面へ"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-7 space-y-12">
            <section>
              ...
              <div className="grid grid-cols-2 gap-6">
                <button onClick={() => setGameMode('standard')} className={`p-8 rounded-[2rem] font-bold flex flex-col items-start gap-4 transition-all border-2 ${gameMode === 'standard' ? 'bg-indigo-600 text-white border-indigo-600 shadow-2xl scale-[1.02]' : 'bg-white/80 backdrop-blur-md text-slate-400 border-white hover:border-indigo-100 shadow-sm'}`}>
                ...
                <button onClick={() => setGameMode('category')} className={`p-8 rounded-[2rem] font-bold flex flex-col items-start gap-4 transition-all border-2 ${gameMode === 'category' ? 'bg-indigo-600 text-white border-indigo-600 shadow-2xl scale-[1.02]' : 'bg-white/80 backdrop-blur-md text-slate-400 border-white hover:border-indigo-100 shadow-sm'}`}>
            ...
            <div className="grid grid-cols-2 gap-12">
              <section>
                ...
                <div className="flex flex-wrap gap-2">
                  {[1, 2, 3, 4, 5, 6].map(num => (
                    <button key={num} onClick={() => setPlayerCount(num)} className={`w-12 h-12 rounded-2xl font-black transition-all ${playerCount === num ? 'bg-indigo-600 text-white shadow-lg scale-110' : 'bg-white/80 backdrop-blur-md text-indigo-400 hover:bg-indigo-50'}`}>
                      {num}
                    </button>
                  ))}
                </div>
              ...
              <section>
                ...
                <div className="flex flex-wrap gap-2">
                  {[4, 8, 10, 16, 20].map(num => (
                    <button key={num} onClick={() => setPanelConfig(num)} className={`px-4 py-3 rounded-2xl font-black transition-all text-xs ${panelConfig === num ? 'bg-purple-600 text-white shadow-lg scale-110' : 'bg-white/80 backdrop-blur-md text-purple-400 hover:bg-purple-50'}`}>
                      {num}枚
                    </button>
                  ))}
                  <button onClick={() => setPanelConfig('tetris')} className={`px-4 py-3 rounded-2xl font-black transition-all text-xs flex items-center gap-2 ${panelConfig === 'tetris' ? 'bg-orange-500 text-white shadow-lg scale-110' : 'bg-white/80 backdrop-blur-md text-orange-400 hover:bg-orange-50'}`}>
            ...
            <section>
              ...
              <label className="w-full flex items-center gap-6 p-8 bg-white/80 backdrop-blur-md text-indigo-500 rounded-[2rem] cursor-pointer hover:bg-indigo-50 transition-all border-2 border-dashed border-indigo-100 shadow-sm group">
          ...
          <div className="lg:col-span-5 space-y-12">
            <section className="bg-white/80 backdrop-blur-md p-8 rounded-[2.5rem] border border-white">
              ...
              <div className="space-y-3">
                {players.map((p, idx) => (
                  <div key={p.id} className="flex items-center gap-4 bg-white/80 backdrop-blur-md p-2 pr-4 rounded-2xl shadow-sm border border-slate-50">
            ...
            <div className="flex flex-col gap-4">
              <button onClick={() => setGameState('editing')} disabled={quizImages.length === 0} className={`w-full py-6 rounded-[2rem] font-black transition-all flex items-center justify-center gap-3 text-lg ${quizImages.length === 0 ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-white/80 backdrop-blur-md border-2 border-indigo-600 text-indigo-600 hover:bg-indigo-50 shadow-xl'}`}>
